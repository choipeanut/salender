...
const computeDelayMs = (attempt: number, policy: RetryBackoffPolicy): number => {
  const base = policy.initialDelayMs * policy.factor ** Math.max(0, attempt - 1);
  const bounded = clamp(base, 0, policy.maxDelayMs);
  ...
  return Math.max(0, Math.round(bounded + offset));
};
